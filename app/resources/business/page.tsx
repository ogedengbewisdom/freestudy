  ...
  return (
    <div className="my-9 w-full">
      <h1 className="text-[2rem] font-bold font-jakarta text-center mb-8 text-[#8B2AFF]">
        Read Business From Your Best Selling Authors
      </h1>
      <h3 className="text-sm font-jakarta text-center mb-8 text-[#6b7280]">
        Business is a field of study that deals with the management,
        organization, and operation of businesses. It includes the study of
        economics, finance, marketing, and management. Business is often used to
        describe the activities of a company or organization.
      </h3>
      ...
      <section className=" w-full">
        <div className=" w-full px-4 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 items-center justify-center gap-4">
          {MOCK_BOOKS_DATA.map((book) => (
            <BookCard
              href={`/resources/business/${book.id}`}
              key={book.id}
              {...book}
            />
          ))}
