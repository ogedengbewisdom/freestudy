...
const PhysicalSciencePage = () => {
  return (
    <div className="my-9 w-full">
      <h1 className="text-[2rem] font-bold font-jakarta text-center mb-8 text-[#8B2AFF]">
        Read Physical Science From Your Best Selling Authors
      </h1>
      <h3 className="text-sm font-jakarta text-center mb-8 text-[#6b7280]">
        Physical science is a branch of science that deals with the study of
        matter and energy. It includes the study of physics, chemistry, and
        biology. Physical science is often used to describe the activities of a
      ...
      <section className=" w-full">
        <div className=" w-full px-4 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 items-center justify-center gap-4">
          {MOCK_BOOKS_DATA.map((book) => (
            <BookCard
              href={`/resources/physical-science/${book.id}`}
              key={book.id}
              {...book}
            />
          ))}
...
export default PhysicalSciencePage;
